...
model = KMeans(n_clusters = 3, max_iter = 15,tol = 0.1,random_state = 100)
...
for k in range(2, 11):
    print ("K=", k )
    kmeans = KMeans(n_clusters=k,max_iter = 100,tol = 0.001,random_state = 100)
    kmeans.fit(X)
    #hcluster= hclust(linkage="ward" ,  n_clusters=k).fit(X)
    label = kmeans.labels_; #label_h = hcluster.labels_
    sl = silhouette_score(X, label, metric='euclidean')
    sil_coeff.append( sl)
    ...
    #print("For n_clusters={}, The H_clustering Silhouette Coefficient is {}".format(k, sil_coeff_hclust))
    print("For n_clusters={}, The K_means Silhouette Coefficient is {} and cost {}  ".format(k, sil_coeff, inertia_kmeans))
...
final_model = KMeans(n_clusters= 3,max_iter = 10,tol = 0.1,random_state = 100 )    
final_model.fit(X)    
